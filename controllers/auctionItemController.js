...
//after declaring auction schema we design its functions adding their routes in appointmentrouter and created a middleware also track Commission Status
         ...
         if(!title||
            ...
            try{
                const cloudinaryResponse=await cloudinary.uploader.upload(
                    image.tempFilePath,
                    {
                        folder:"MERN_AUCTION_PLATFORMS_AUCTIONS",
                    }     
                );
...
export const getAuctionDetails=catchAsyncErrors (async (req,res,next)=>{
    const {id}=req.params;   //routes me forward slash ke baad jo id ki value hogi usse get kr rhe hai 
...
export const republishItem=catchAsyncErrors(async (req,res,next)=>{
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      ...
      if(!req.body.startTime || !req.body.endTime){
        return next(new ErrorHandler("Startime and EndTime for republish is mandatory.",400));
      }
      ...
      if(data.startTime>=data.endTime){
        return next(new ErrorHandler("Auction must end after start date",400));
      }
